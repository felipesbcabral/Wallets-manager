...
export default function Esqueci(props: EsqueciSenhaScreenProps) {
  const [email, setEmail] = useState('');

  const { height } = useWindowDimensions();
  const signIn = () => props.navigation.navigate('Login');
  const forgotPassword = () => props.navigation.navigate('Esqueci');
  const createAccount = () => props.navigation.navigate('Cadastro');
  return (
    <LinearGradient
      colors={['#94FC13', '#4BE3AC', '#94FC13']}
      style={styles.body}
      start={{ x: 1.0, y: 1.0 }}
      end={{ x: 0.0, y: 0.0 }}>
      <View>
        <View style={styles.walletsIconConfig}>
          <Image
            ...
            source={require('../assets/wallets-icon.png')}
          />
        </View>
      ...
      <CustomButton text="Enviar" onPress={signIn} />
      <CustomButton
          text="Voltar"
        onPress={signIn}
        />
